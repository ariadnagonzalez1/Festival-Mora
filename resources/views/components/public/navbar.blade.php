<header class="fixed top-0 left-0 right-0 z-50 border-b border-white/10 bg-[#070716]/90 backdrop-blur-xl">
    <div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav class="hidden items-center gap-8 md:flex">
            ...
            @auth
                @if(auth()->user()->esUsuario())
                    <a href="{{ route('user.mis-entradas') }}"
                       class="text-sm font-semibold transition {{ request()->routeIs('user.mis-entradas') ? 'text-white' : 'text-slate-400 hover:text-white' }}">
                        Mis entradas
                    </a>
                @endif
            @endauth
        ...
        <div class="hidden items-center gap-4 md:flex">
            @auth
                <span class="rounded-xl border border-white/10 bg-white/[0.04] px-4 py-2 text-sm font-semibold text-slate-300">
                    {{ auth()->user()->nombre }}
                </span>
                ...
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <button
                        type="submit"
                        class="rounded-xl border border-white/10 px-4 py-2 text-sm font-bold text-white transition hover:bg-white/10"
                    >
                        Salir
                    </button>
                </form>
        ...
        <div class="md:hidden">
            @auth
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <button
                        type="submit"
                        class="rounded-lg border border-white/10 px-3 py-2 text-sm font-bold text-white transition hover:bg-white/10"
                    >
                        Salir
                    </button>
                </form>
