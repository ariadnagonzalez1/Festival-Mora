<aside class="fixed left-0 top-0 z-40 hidden h-screen w-64 border-r border-white/10 bg-[#070716] lg:block">
   ...
   {{ request()->routeIs('admin.ventas') ? 'bg-violet-600/30 text-white ring-1 ring-white' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
    ...
    Ventas
            ...
            <a href="{{ route('admin.validar-qr') }}"
   class="flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-bold transition
   {{ request()->routeIs('admin.validar-qr') ? 'bg-violet-600/30 text-white ring-1 ring-white' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
    <span>⌗</span>
    Validar QR
</a>
   ...
   class="flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-bold transition
   {{ request()->routeIs('admin.usuarios') ? 'bg-violet-600/30 text-white ring-1 ring-white' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
    <span>◌</span>
    Usuarios
</a>
